...
def load_snli_data_for_training():
    """Load SNLI dataset with large sample sizes"""
    ...
    print("Converting TensorFlow datasets to pandas DataFrames...")
...
def generate_enhanced_embeddings_tf(
    premises, hypotheses, use_model, batch_size=512, save_path=None
):
    """Generate enhanced embeddings using TensorFlow Universal Sentence Encoder in batches, optionally saving/loading"""
    if save_path and os.path.exists(save_path):
        print(f"Loading cached embeddings from {save_path}...")
        return np.load(save_path)

    print(
        f"Generating enhanced embeddings for {len(premises)} samples using USE in batches..."
    )
    ...
    with tf.device("/CPU:0"):
        for i in tqdm(
            range(0, len(premises), batch_size), desc="Generating USE embeddings"
        ):
            batch_premises = premises[i : i + batch_size]
            batch_hypotheses = hypotheses[i : i + batch_size]

            # Convert to TensorFlow tensors
            premise_tensor = tf.constant(batch_premises)
            hypothesis_tensor = tf.constant(batch_hypotheses)

            # Generate embeddings
            premise_embeddings = use_model(premise_tensor)
            hypothesis_embeddings = use_model(hypothesis_tensor)
...
def train_tensorflow_student(
    ...
    for epoch in range(epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            convert_to_tflite(trained_model_path, output_dir)
        else:
    ...
    if os.path.exists(os.path.join(output_dir, "best_student_model.keras")):
...
def train_cross_framework_distilled_nli(
    ...
    try:
        train_df, val_df, test_df = load_snli_data_for_training()
    ...
    del train_soft_labels, train_hard_labels, val_soft_labels, val_hard_labels
    import gc  # Garbage Collection

    gc.collect()

    print(f"\n=== Phase 3: Loading TensorFlow USE Model ===")
    try:
        use_model = hub.load(use_model_url)
        print("TensorFlow USE model loaded successfully.")
    except Exception as e:
        print(f"Error loading USE model: {e}")
        return

    print(f"\n=== Phase 4: Generating Enhanced Embeddings with TensorFlow USE ===")
    ...
    input_dim = int(
        X_test.shape[1]
    )  # Load a small portion or the whole test set for this.
    # Alternatively, infer from USE model output if fixed.
    ...
    # Load X_test for evaluation (it was kept previously, but if you removed it, load it here)
    X_test_eval = np.load(
