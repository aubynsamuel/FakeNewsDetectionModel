...
class OptimizedFakeNewsDetector:
    ...
    def prepare_features(self, texts, labels=None):
        ...
        if self.use_distilbert:
            if len(texts) > self.max_bert_samples:
                print(
                    f"Sampling {self.max_bert_samples} texts for DistilBERT (dataset too large)"
                )
                indices = np.random.choice(
                    len(texts), self.max_bert_samples, replace=False
                )
    ...
    def save_artifacts(self, prefix="optimized"):
        ...
        metadata = {
            ...
            "model_results": {
                name: result["metrics"] for name, result in self.models.items()
            },
