...
class AdvancedClickbaitDetector:
    ...
    def __init__(self):
        ...
        self.suspicious_patterns = [
            ...
            (r'\b(miracle|instant|amazing|incredible)\s+(cure|results?|discovery)\b', 0.4),
    ...
    def extract_handcrafted_features(self, texts):
        ...
        for text in texts:
            ...
            if text:
                ...
                word_caps_ratio = sum(1 for word in text.split() if word.isupper()) / max(len(text.split()), 1)
            else:
            ...
            clickbait_words = ['amazing', 'shocking', 'unbelievable', 'incredible', 'stunning', 
                             'secret', 'hidden', 'revealed', 'exposed', 'trick', 'hack']
            clickbait_word_count = sum(1 for word in clickbait_words if word in text_lower)
    ...
    def load_and_prepare_data(self):
        ...
        clickbait_csv_path = project_root_dir / 'data' / 'fixed_clickbait_data.csv'
        non_clickbait_csv_path = project_root_dir / 'data' / 'fixed_non_clickbait_data.csv'
        
        # Load clickbait data
        try:
            clickbait_df = pd.read_csv(clickbait_csv_path, header=None, engine='python')
            clickbait_texts = clickbait_df.iloc[:, 0].astype(str).tolist()  # First column
        ...
        try:
            non_clickbait_df = pd.read_csv(non_clickbait_csv_path, header=None, engine='python')
            non_clickbait_texts = non_clickbait_df.iloc[:, 0].astype(str).tolist()  # First column
    ...
    def train(self):
        ...
        # Cross-validation
        cv_scores = cross_val_score(self.ensemble_model, X_train_combined, y_train, cv=5, scoring='roc_auc')
    ...
    def load_model(self):
        ...
        try:
            ...
            print("Model loaded successfully from models/advanced_clickbait_detector.pkl")
            return True
        except Exception as e:
    ...
    def detect_clickbait_score(self, headline: str) -> float:
        ...
        try:
            ...
            # Combine features
            from scipy.sparse import hstack, csr_matrix
            combined_features = hstack([tfidf_features, csr_matrix(handcrafted_features)])
            
            # Get prediction probability
            clickbait_probability = self.ensemble_model.predict_proba(combined_features)[0][1]
