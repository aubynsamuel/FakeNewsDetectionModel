...
def predict_fake_news(text, model_dir="models/"):
    ...
    try:
        ...
        if not os.path.exists(metadata_path):
            return {'error': f"Metadata file not found: {metadata_path}. Please run the training script first."}
        
        with open(metadata_path, 'rb') as f:
            metadata = pickle.load(f)
        
        use_distilbert = metadata.get('use_distilbert', True)
        best_model_name = metadata.get('best_model_name', 'XGBoost').lower().replace(" ", "_")
        
        # Load the Best Trained Model
        model_path = os.path.join(model_dir, f'optimized_v2_best_model_{best_model_name}.pkl')
        if not os.path.exists(model_path):
            return {'error': f"Best model file not found: {model_path}. Did you train the model successfully?"}
        ...
        if not processed_text:
            return {'prediction': 'Uncertain', 'confidence': 0.5, 'message': 'Input text was too short or cleaned to an empty string after preprocessing.'}
        ...
        if use_distilbert:
            ...
            if not os.path.exists(config_path):
                 return {'error': f"DistilBERT config file not found: {config_path}. Ensure DistilBERT was used in training."}
            ...
            if not os.path.exists(tfidf_path):
                return {'error': f"TF-IDF vectorizer file not found: {tfidf_path}. Ensure TF-IDF was used in training."}
        ...
        return {
            ...
            'probabilities': {'fake': float(prediction_proba[0]), 'true': float(prediction_proba[1])},
            'processed_text_sample': processed_text[:200] + '...' if len(processed_text) > 200 else processed_text
        }
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if 'error' in result:
            ...
            print(f"Probabilities - Fake: {result['probabilities']['fake']:.4f}, True: {result['probabilities']['true']:.4f}")
